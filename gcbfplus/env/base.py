...
class StepResult(NamedTuple):                   # Define a StepResult custom tuple to store the call of one step
    graph: GraphsTuple
    reward: Reward
    cost: Cost
    done: Done
...
class RolloutResult(NamedTuple):                # Bundles an entire episode with arrays size of T
...
class MultiAgentEnv(ABC):
    ...
    def __init__(
        ...
        self._dt = dt
        if params is None:                          # Do this because params is a dict type and is mutable
            params = self.PARAMS
        self._params = params
        self._t = 0
    ...
    @abstractmethod
    @ft.partial(jax.jit, static_argnums=(0,))                       # Returns a new function object that behaves like jax.jit with self fixed
    ...
    def rollout_fn(self, policy: Callable, rollout_length: int = None) -> Callable[[PRNGKey], RolloutResult]:       #
        rollout_length = rollout_length or self.max_episode_steps                                                   # If no rollout length is specified use max steps

        def body(graph, _):                                                                                         # Unused parameter _: "I have to accept this argument for the API, but I’m not going to use it"
            action = policy(graph)                                                                                  # Calculate the action based on current graph
            graph_new, reward, cost, done, info = self.step(graph, action, get_eval_info=True)                      # Unpack step return
        ...
        def fn(key: PRNGKey) -> RolloutResult:
            ...
            graph_final, (T_graph, T_action, T_reward, T_cost, T_done, T_info) = lax.scan(
                body,                       # the per‐step function
                graph0,                     # initial “carry”
                None,                       # the “sequence” (unused here)
                length=rollout_length
            )                               # lax.scan(f, init, xs, length=L) returns a pair (carry_L, ys)
    ...
    def rollout_fn_jitstep(
        ...
        is_unsafe_fn = jax_jit_np(self.collision_mask)                  # Convert to Numpy array
