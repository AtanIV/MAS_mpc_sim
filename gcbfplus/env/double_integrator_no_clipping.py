...
class DoubleIntegratorNoClipping(MultiAgentEnv):
    AGENT = 0
    GOAL = 1
    OBS = 2
    ...
    PARAMS = {
        ...
        "comm_radius": 0.5,
        "n_rays": 32,
        # "obs_len_range": [0.1, 0.5], # original
    ...
    def __init__(
        ...
        super(DoubleIntegratorNoClipping, self).__init__(num_agents, area_size, max_step, max_travel, dt, params)
        A = np.zeros((self.state_dim, self.state_dim), dtype=np.float32)                    # Initialize state matrix
        A[0, 2] = 1.0                                                                       # State matrix 3*3
        A[1, 3] = 1.0
    ...
    def get_graph(self, state: EnvState, adjacency: Array = None) -> GraphsTuple:
        ...
        node_type = node_type.at[self.num_agents: self.num_agents * 2].set(DoubleIntegratorNoClipping.GOAL)
        node_type = node_type.at[-n_hits:].set(DoubleIntegratorNoClipping.OBS)
    ...
    def state_lim(self, state: Optional[State] = None) -> Tuple[State, State]:
        ...
        # lower_lim = jnp.array([-jnp.inf, -jnp.inf, -10, -10])
        # upper_lim = jnp.array([jnp.inf, jnp.inf, 10, 10])
