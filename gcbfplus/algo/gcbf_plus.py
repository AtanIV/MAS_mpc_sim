...
class Batch(NamedTuple):    # Container to hold minibatch
    graph: GraphsTuple      # JAX-friendly data structure to store agent graphs?
    safe_mask: Array        # Bool array labeling which agent-timesteps are safe
    unsafe_mask: Array      # Bool array labeling which agent-timesteps are unsafe
    u_qp: Action            # Baseline control input from the QP


class GCBFPlus(GCBF):       # Subclass of GCBF?

    def __init__(           # Initializing parameters
            self,
            env: MultiAgentEnv,
            node_dim: int,
            edge_dim: int,
