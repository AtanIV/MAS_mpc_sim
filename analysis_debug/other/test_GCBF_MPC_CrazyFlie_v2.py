...
class CrazyflieSafetyMPC:
    """MPC controller for CrazyFlie with GCBF safety constraints and velocity damping near goals."""
    ...
    def __init__(self,
                 ...
                 # New parameters for velocity damping
                 velocity_damping_zone: float = 0.8,  # Start damping within this distance (m)
                 min_velocity_limit: float = 0.1,  # Minimum velocity limit near goal (m/s)
                 velocity_penalty_weight: float = 10.0,  # Weight for velocity penalty near goal
                 goal_tolerance: float = 0.05):  # Consider "at goal" within this distance
        ...
        self.velocity_damping_zone = velocity_damping_zone
        self.min_velocity_limit = min_velocity_limit
        ...
        print(f"  Velocity damping zone: {velocity_damping_zone} m")
        print(f"  Min velocity limit: {min_velocity_limit} m/s")
        print(f"  Velocity penalty weight: {velocity_penalty_weight}")
    ...
    def _solve_single_agent(self, current_state: np.ndarray, goal_state: np.ndarray,
        ...
        """Solve MPC for single CrazyFlie agent with velocity damping near goal."""
        ...
        # If already very close to goal, just hover
        if dist_to_goal < self.goal_tolerance:
            hover_action = np.zeros(4)
            return hover_action, "HOVERING_AT_GOAL", True

        # Calculate proximity factor (0 = far from goal, 1 = at damping zone boundary)
        proximity_factor = max(0.0, min(1.0, (self.velocity_damping_zone - dist_to_goal) / self.velocity_damping_zone))
        ...
        for k in range(self.horizon):
            ...
            # ===== APPROACH 3: ADAPTIVE VELOCITY CONSTRAINTS (FIXED VERSION) =====
            # Use current proximity_factor (based on current state, not predicted)
            # This keeps the constraint convex by using fixed parameters
            velocity_limit_factor = max(
                self.min_velocity_limit / self.v_max,
                min(1.0, 1.0 - proximity_factor * 0.9)  # Reduce limit as we get closer
            )

            adaptive_v_max = self.v_max * velocity_limit_factor

            # Apply adaptive velocity constraint (now convex)
            constraints += [cp.norm(u[k, :3], 2) <= adaptive_v_max]

            # Standard yaw rate limit (not affected by proximity)
            constraints += [cp.abs(u[k, 3]) <= self.r_max]

            # ===== APPROACH 2: DISTANCE-BASED VELOCITY PENALTY (FIXED VERSION) =====
            # Standard goal-reaching objective (position tracking)
            cost += self.goal_weight * cp.quad_form(x[k] - goal_state, self.Q)

            # Standard control effort penalty
            cost += cp.quad_form(u[k], self.R)

            # Additional velocity penalty that increases when close to goal
            # This creates a soft constraint encouraging slower motion near the goal
            velocity_magnitude_squared = cp.sum_squares(u[k, :3])  # |vx, vy, vz|^2

            # Penalty weight increases with proximity to goal (using current distance)
            distance_based_penalty = self.velocity_penalty_weight * proximity_factor
            cost += distance_based_penalty * velocity_magnitude_squared

            # Simplified approach penalty using current goal direction (fixed, not variable)
            # Calculate goal direction from current state (not decision variables)
            current_to_goal = goal_pos - current_pos
            current_to_goal_norm = np.linalg.norm(current_to_goal)

            if current_to_goal_norm > 1e-4:
                goal_direction_fixed = current_to_goal / current_to_goal_norm
                # Velocity component in goal direction (now convex)
                velocity_toward_goal = goal_direction_fixed @ u[k, :3]

                # Extra penalty for velocity toward goal when close
                approach_penalty = self.velocity_penalty_weight * proximity_factor * 2.0
                cost += approach_penalty * velocity_toward_goal ** 2  # Quadratic penalty (always convex)
        ...
        for solver, solver_name in zip([cp.OSQP, cp.ECOS, cp.SCS], solver_names):
            try:
                ...
                if prob.status in ["optimal", "optimal_inaccurate"]:
                    feasible = True
                    if step == 0 and proximity_factor > 0.1:  # Debug info when damping is active
                        print(f"    Agent {agent_idx}: MPC solved ({solver_status}) - "
                              f"dist={dist_to_goal:.3f}m, prox_factor={proximity_factor:.3f}, "
                              f"v_limit={adaptive_v_max:.3f}")
        ...
        # If all solvers fail, use a simple proportional controller as fallback
        print(f"  Agent {agent_idx}: MPC infeasible ({solver_status}) - using P controller fallback")

        # Simple proportional controller with velocity limiting
        pos_error = goal_pos - current_pos
        vel_error = -current_state[6:9]  # Negative body velocities (assuming we want to stop)

        # Proportional control
        kp_pos = 1.0
        kd_vel = 0.2

        desired_vel = kp_pos * pos_error + kd_vel * vel_error

        # Apply velocity limits based on distance
        vel_magnitude = np.linalg.norm(desired_vel)
        if vel_magnitude > 0:
            max_allowed_vel = max(self.min_velocity_limit,
                                  self.v_max * max(0.1, min(1.0, dist_to_goal / self.velocity_damping_zone)))
            if vel_magnitude > max_allowed_vel:
                desired_vel = desired_vel * (max_allowed_vel / vel_magnitude)

        fallback_action = np.array([desired_vel[0], desired_vel[1], desired_vel[2], 0.0])

        return fallback_action, f"FALLBACK_P_CONTROLLER", False
...
def test_crazyflie_gcbf_mpc(args):
    ...
    mpc_controller = CrazyflieSafetyMPC(
        ...
        # New velocity damping parameters
        velocity_damping_zone=args.velocity_damping_zone,
        min_velocity_limit=args.min_velocity_limit,
...
def main():
    ...
    # Velocity damping parameters
    parser.add_argument("--velocity-damping-zone", type=float, default=0.8,
                        help="Distance from goal to start velocity damping (m)")
    parser.add_argument("--min-velocity-limit", type=float, default=0.1,
                        help="Minimum velocity limit near goal (m/s)")
    parser.add_argument("--velocity-penalty-weight", type=float, default=10.0,
                        help="Weight for velocity penalty near goal")
    parser.add_argument("--goal-tolerance", type=float, default=0.05,
                        help="Distance threshold to consider 'at goal' (m)")
